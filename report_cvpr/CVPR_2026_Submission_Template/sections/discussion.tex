...
The final model's behavioral instability seems to be supported by the critic-loss spikes.
However, these spikes are not necessarily indicative of unstable training, since reinforcement-learning targets evolve together
with the policy: when the actor discovers better trajectories, previously learned value estimates become inaccurate,
...
more advanced actor-critic methods for continuous control, such as TD3 \cite{Fujimoto2018}.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.72\columnwidth]{imgs/sensitivity_return.png}
