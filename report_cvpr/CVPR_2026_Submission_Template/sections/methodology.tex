...
Part 1 focused on learning a locomotion policy for the Hopper environment by comparing three policy-gradient methods: REINFORCE, REINFORCE with baseline, and Actor-Critic architectures \cite{SuttonBarto2018}.
All models utilized a 2-layer MLP with hidden sizes [64, 64] and Tanh activations, trained using the Adam optimizer with learning rate $3 \times 10^{-4}$ and discount factor $\gamma = 0.99$.

REINFORCE \cite{Williams1992} is the simplest policy algorithm, updating the policy parameters at the end of each episode using the full Monte Carlo return.
In the first variant $b=0$ we have vanilla REINFORCE. In the second, the return is shifted by a constant baseline $b = 20$.

We used orthogonal initialization \cite{saxe2013exact} instead of the standard normal weight initialization for better training stability.
The hidden layers used a gain of $\sqrt{2}$ to preserve the gradient norms and avoid exploding or vanishing gradients.
Furthermore, the output layer of the actor was initialized with a low gain of 0.01.
This makes sure the initial policy outputs action values close to zero, stopping extreme movements that would destabilize the Hopper right away.
Therefore, the early exploration is entirely governed by the learned standard deviation, which allows the agent to survive for a longer period of time and collect better trajectories during the initial training stage \cite{andrychowicz2020matters}.
...
deviation of a Gaussian action distribution $a_t \sim \mathcal N(\mu_\theta(s_t), \sigma_\theta(s_t))$, where both parameters are learned by the policy network.
Actions are sampled during training to encourage exploration, while the mean action $\mu_\theta(s_t)$ is used during evaluation.

To improve the quality of the policy updates, Generalized Advantage Estimation (GAE)
was implemented \cite{SchulmanGAE2015}, which computes the advantage function as a weighted sum of n-step returns, controlled by the $\lambda$ parameter.

Subsequently, advantages were normalized before policy updates in order to stabilize gradients,
improve the critic's estimates and reduce training instability. A minimum standard deviation (sigma floor)
...
The goal was to reduce the update magnitude once the Hopper had likely discovered jumping, as this behaviour leads to
...
Proximal Policy Optimization (PPO) \cite{SchulmanPPO2017} is an on-policy actor-critic algorithm that limits policy updates through a clipped objective.
The idea behind this algorithm is that excessively large policy updates result in unstable training, so stability is achieved by keeping updates within a trust region.
...
Soft Actor-Critic (SAC) \cite{SAC2018} is an off-policy actor-critic method that maximizes both reward and policy entropy.

By combining a replay buffer, twin critics to reduce value overestimation, and automatic entropy tuning, 
SAC is able to generally provide better exploration and sample efficiency than PPO, especially in continuous-control robotic tasks.
...
The final SAC configuration is reported in Table~\ref{tab:sac_configuration}; observations and rewards were normalized with \texttt{VecNormalize}.

\begin{table}[ht]
\centering
\caption{Final SAC Configuration}
\label{tab:sac_configuration}
